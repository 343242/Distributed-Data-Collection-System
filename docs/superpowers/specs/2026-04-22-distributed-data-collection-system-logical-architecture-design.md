...
| v0.7 | 2026-04-23 | 将部署前提从单客户单实例修订为多租户共享中心平台，并引入租户隔离作为统一架构约束 |
| v0.8 | 2026-04-23 | 补齐消息与失败隔离字段中的租户维度，使逻辑架构与详细设计公共模型保持一致 |
...
- 支撑统一中心平台服务多个客户，每个客户现场可部署多个 `Client Agent`
...
| 需求 | 摘要 | 对应章节 | 设计落实说明 |
| --- | --- | --- | --- |
| R1 | 多客户平台下多 Agent 现场部署 | 4、6、7、8、10、14、23、24 | 明确多租户共享中心平台、租户绑定的 Agent 注册、Agent 管理、租户隔离和横向扩展 |
...
本系统是一个统一中心平台，由一套共享的 `Business System + Data Gateway + Data Storage` 服务多个客户。部署边界定义如下：

- 中心侧是一套共享平台实例，而不是一个客户一套中心系统
- 不同客户以 `Tenant` / `tenantId` 作为平台内的一级隔离边界
- 每个客户在现场可部署一个或多个 `Client Agent`
- 每个 `Client Agent` 只能归属一个 `Tenant`

因此，本方案必须显式引入平台型 `Tenant` 模型，而不是依赖物理部署边界来完成客户隔离。任务、Agent、实例、消息、审计、监控和存储记录都必须在租户维度下隔离。

### 4.2 总体架构概述

系统采用“分布式采集、集中治理、统一接入、统一存储”的总体逻辑架构。

多个客户的现场分别部署一个或多个 `Client Agent`。每个 `Agent` 在注册时绑定所属 `Tenant`，并根据中心平台下发的目标配置，在本机托管运行多个 `Connector` 实例。`Connector` 负责连接外部数据提供服务器并执行实际采集。采集结果先进入 `Agent` 本地持久化队列，再上传到共享中心平台中的 `Data Gateway`。`Data Gateway` 负责统一控制、统一数据接入、统一处理和统一入库，最终由 `Data Storage` 落地并以租户隔离方式供 `Business System` 查询和消费。
...
`Tenant` 表示平台中的客户隔离边界。一个 `Tenant` 代表一个客户在中心平台中的独立逻辑空间。

### 6.15 Tenant Scope

`Tenant Scope` 表示对象在租户维度上的归属和可见范围。第一版要求以下对象必须具备明确的 `tenantId`，或能够由身份上下文可靠推导出所属租户：

- `Task`
- `Client Agent`
- `Connector Instance`
- `Connector Package` / 配置
- `Command`
- `Message`
- 审计日志
- 监控指标
...
- 同一租户内的任务或 Agent 目标状态变更必须串行化提交，避免冲突命令

第一版 `Control Plane` 写路径明确采用“单 Leader + 共享元数据存储”策略，由 Leader 负责目标状态变更、命令生成和版本推进，以降低多实例协调复杂度。
...
`Business System` 作为共享中心平台入口，必须在租户维度下提供任务、运行态、审计和查询能力。它不负责具体执行调度、程序包管理和现场运行恢复。
...
- 为业务查询、历史追溯和后续消费提供支撑
- 在共享中心平台中按租户维度隔离数据、失败记录、审计和查询结果
...
系统部署在多租户共享中心平台、客户现场 Agent 通过专线 / VPN / 内网互通接入的前提下，但仍需具备基础安全能力。
...
- Gateway 仅接受已登记且租户归属校验通过的 Agent 请求
...
- 节点操作通过 Agent 身份体系执行
- 所有控制面查询、写操作、审计查看和数据查询都必须受租户范围约束

### 10.4 审计要求

- 任务、配置、程序包、命令、迁移、Agent 注册与失效等关键操作必须记录审计日志
- 运行日志中不得明文记录敏感凭证
- 审计日志必须包含 `tenantId` 或可可靠推导租户归属的上下文
...
- 所有控制对象、数据对象和审计对象都属于唯一 `Tenant`
- `Data Gateway` 只允许在租户范围内控制 `Client Agent`
- `Business System` 只允许在租户范围内查看任务、状态、审计和数据
...
三条链路中的对象流转都必须携带租户归属，或能够由节点身份、任务上下文和目标状态可靠映射出所属 `Tenant`。
...
1. 携带节点身份发起注册
2. Gateway 校验节点身份、租户归属和所属部署
3. Agent 上报基础信息，例如 `agentId`、`tenantId`、版本、能力和主机标识
...
`tenantId` 是多租户共享中心平台下消息隔离、接入校验、处理路由、失败隔离和最终查询过滤的必备字段，或必须能够由 Agent 身份与任务上下文可靠推导并在进入中心处理链前补齐。
...
- `Durable Ingest Storage` 采用具备持久化和可重放能力的消息日志 / 队列型存储
- `Business Query Storage` 采用支持事务、唯一约束和 `UPSERT` 语义的事务型存储
- 若需补充分析型查询能力，应作为后续独立下游副本扩展，而不替代上述两层职责
...
默认离线策略明确采用 `ContinueUntilWatermark`。
...
动态分发在架构上保留完整演进路径，但不纳入第一版核心必交付范围。
...
即使项目在第一版内追加动态分发增强能力，以下能力也明确不纳入支持范围：

- 双活抢占式迁移
- 同一采集流多 Agent 并行消费
- 无停机热迁移

第一版默认不实现动态分发执行能力，仅保留：

- `Rebalancing` 状态与迁移语义预留
- `Checkpoint` 交接语义
- 单主归属约束
...
Agent 定期上报实例级摘要，Gateway 聚合展示系统级、租户级、Agent 级、任务级和实例级视图。
...
支持一个中心平台服务多个客户，每个客户部署多个 Agent，支持多种 `Connector Type`、多程序包版本和多实例横向扩展。
