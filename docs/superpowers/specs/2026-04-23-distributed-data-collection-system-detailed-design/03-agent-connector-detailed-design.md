...
| v0.5 | 2026-04-23 | 补充多租户共享中心平台下的 Agent 租户绑定、租户上下文传递和租户范围约束 |
| v0.6 | 2026-04-23 | 修正恢复与异常处理章节编号顺序，消除文档结构不一致 |
...
4. Gateway 校验身份、租户归属、部署边界以及旧身份失效状态
5. Agent 上报基础信息、`tenantId`、能力摘要与运行实例标识
...
- Agent 只能绑定一个 `tenantId`，不允许在运行中切换租户归属

#### 3.2.2 目标状态收敛流程

1. Agent 获取最新 `desiredStateVersion`
2. 校验目标状态中的 `tenantId` 与本地租户归属一致
3. 对比本地已应用版本
4. 生成本地动作集合
5. 顺序执行程序包、配置、实例管理动作
6. 回报结果
...
2. Agent 执行本地原子提交
3. 消息进入本地持久化队列
4. Upload Worker 携带租户上下文批量发送到 Gateway
...
| --- | --- |
| sourceRecord | 源记录或批次 |
| tenantId | 租户标识，或由 Agent 运行上下文补齐 |
