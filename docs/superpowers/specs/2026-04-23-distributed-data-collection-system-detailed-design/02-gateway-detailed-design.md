...
| v0.9 | 2026-04-23 | 对齐公共接口基线，补齐失败查询请求租户字段并修正路由职责重复表述 |
...
- Gateway 返回 `Accepted` 的前提是数据已经进入中心可靠接入层，而不是已经写入最终查询库
- 最终查询库存放的是可查询、可消费的数据结果，不应承担中心第一落点职责
- 系统允许因为重试产生重复投递，但最终查询库存储必须具备幂等生效能力
...
6. 执行基于 `recordDedupKey` 的业务层去重
7. 执行路由决策
8. 调用 Storage Writer 写入最终查询存储
9. 记录处理结果和时间语义
10. 对失败消息执行重试、退避或隔离
11. 标记处理完成或转入失败隔离区
