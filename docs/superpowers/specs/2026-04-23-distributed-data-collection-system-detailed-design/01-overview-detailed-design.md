...
下列概念定义以上游逻辑架构方案为准，本轮详细设计仅在此做最小索引，避免四份详细设计文档出现术语漂移。
...
| 需求 | 摘要 | 详细设计文档 | 设计落实说明 |
| --- | --- | --- | --- |
| `R1` | 单客户下多 Agent 部署 | 01、02、03 | 在总览中明确部署边界，在 Gateway 与 Agent 文档中展开注册、管理与运行关系 |
...
| `R5` | 数据必须入库 | 01、02、04 | 在总览中固定数据链路，在 Gateway 文档中展开处理链路，在公共模型文档中定义消息与响应语义 |
...
- 运行日志不得明文记录敏感凭证
- 任务、配置、程序包、命令、迁移、身份变更等关键动作必须纳入审计范围
...
- 监控至少覆盖系统级、Agent 级、Task 级和 `Connector Instance` 级四层粒度
- 审计日志至少记录操作类型、操作对象、发起者、发起时间、目标版本和执行结果
...
建议实现顺序如下。该顺序与逻辑架构 `v0.3` 的“第一版实现边界”保持一致：Phase 1-3 对应首版建议实现范围，Phase 4 对应增强能力预留。
