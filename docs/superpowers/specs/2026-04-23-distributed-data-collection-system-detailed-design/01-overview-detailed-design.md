...
- 本次修订先统一多租户共享中心平台这一总前提
- `02/03/04/05` 中涉及租户字段、租户范围校验和租户隔离落地的细节将在后续文档修订中继续补齐
...
| `Tenant` | 平台中的客户隔离边界 | 01，后续 02/04/05 待同步细化 |
| `tenantId` | 租户归属标识，控制、数据、审计与存储的统一隔离维度 | 01，后续 02/04/05 待同步细化 |
